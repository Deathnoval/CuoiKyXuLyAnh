...
def process_video():
    ...
    while True:
        ...
        if not ret:
            break
        # Process the frame (replace this with your own video processing code)
        processed_frame = frame
        
		
        # Display the processed frame on the GUI3
        img = PIL.Image.fromarray(processed_frame)
        img=img.resize((800, 600), resample=Image.BILINEAR)
        img_rgb = img.convert('RGB')
        img_tk = PIL.ImageTk.PhotoImage(img_rgb, gamma=2.0)
...
name_label4 =tk.Label(root,text="   Họ và Tên, MSSV THỰC HIỆN ĐỀ TÀI",fg="RED",font=("Arial", 12))
name_label4.grid(row=1,column= 5)
name_label1 =tk.Label(root,text="    ĐỊNH QUÂN 20110043",fg="RED",font=("Arial", 12))
name_label1.grid(row=2,column= 5)
name_label2 =tk.Label(root,text="    LÊ CÔNG TRÌNH 20110408",fg="RED",font=("Arial", 12))
name_label2.grid(row=3,column= 5)
name_label3 =tk.Label(root,text="    ĐỖ THỊ BÍCH NGỌC 20110526",fg="RED",font=("Arial", 12))
name_label3.grid(row=4,column= 5)
#TẠO LABEL CHO LOGO
...
process_button = tk.Button(root, text="Process Video", command=process_video,width=15, height=2)
